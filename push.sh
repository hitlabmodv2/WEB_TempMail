...
action_switch_repo() {
  ...
  local _sr_raw _sr_http
  _sr_http=$(curl -s \
    -o /tmp/_gh_sr_list.json \
    -w "%{http_code}" \
    -H "Authorization: token ${TOKEN}" \
    -H "Accept: application/vnd.github+json" \
    -H "X-GitHub-Api-Version: 2022-11-28" \
    "https://api.github.com/user/repos?per_page=100&sort=updated&affiliation=owner" 2>/dev/null)
  _sr_raw=$(cat /tmp/_gh_sr_list.json 2>/dev/null)

  local repos=()
  # Hanya parse kalau HTTP 200 — hindari parse error JSON dari API
...
try:
  ...
  import sys; print("ERR:"+str(e), file=sys.stderr)
' 2>/tmp/_gh_sr_py_err.txt)
  fi

  local _sr_err_reason=""
  case "$_sr_http" in
    200)  : ;;
    401)  _sr_err_reason="Token tidak valid / sudah kadaluarsa (HTTP 401)" ;;
    403)  _sr_err_reason="Token tidak punya scope 'repo' (HTTP 403)" ;;
    000|"") _sr_err_reason="Tidak ada koneksi internet / GitHub tidak bisa dicapai" ;;
    *)    _sr_err_reason="GitHub menolak permintaan (HTTP ${_sr_http})" ;;
  esac

  if [ "${#repos[@]}" -eq 0 ]; then mini_bar_fail "Tidak ada repo (HTTP ${_sr_http:-???})"; else mini_bar_ok "${#repos[@]} repo dimuat"; fi
  local total=${#repos[@]}
  ...
  if [ "$total" -eq 0 ]; then
    echo -e "  ${C_YELLOW}⚠️  Daftar repo tidak bisa dimuat.${C_RESET}"
    if [ -n "$_sr_err_reason" ]; then
      echo -e "  ${C_RED}   Penyebab: ${_sr_err_reason}${C_RESET}"
      # Saran spesifik per jenis error
      case "$_sr_http" in
        401) echo -e "  ${C_DIM}   → Masuk menu utama lagi untuk input token baru.${C_RESET}" ;;
        403) echo -e "  ${C_DIM}   → Buat token baru dengan centang scope 'repo (full control)'.${C_RESET}" ;;
        000|"") echo -e "  ${C_DIM}   → Cek koneksi internet, lalu coba lagi.${C_RESET}" ;;
      esac
    fi
    echo ""
    echo -e "  ${C_DIM}💡 Ketik langsung${C_RESET} ${C_BOLD}user/repo${C_RESET}${C_DIM} untuk ganti tanpa daftar.${C_RESET}"
    echo -e "  ${C_DIM}   Repo aktif sekarang: ${C_RESET}${C_BOLD}${USER}/${REPO}${C_RESET}"
    echo ""
    echo -e "  ${C_RED} 0${C_RESET} ${C_BOLD}›${C_RESET} Kembali ke menu"
    echo -e "${C_DIM}  ──────────────────────────────────${C_RESET}"
