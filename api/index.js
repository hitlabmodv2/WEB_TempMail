...
app.get('/api/server-info', (req, res) => {
  ...
  const fmt = s => { const d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60),sec=Math.floor(s%60); return {days:d,hours:h,mins:m,secs:sec,raw:Math.floor(s)}; };
  res.json({
    success: true,
    system: { platform: os.platform(), arch: os.arch(), nodeVersion: process.version, v8Version: process.versions.v8 },
    memory: { total: toGB(totalMem)+' GB', used: toMB(usedMem)+' MB', free: toMB(freeMem)+' MB', usedPercent: Math.round(usedMem/totalMem*100) },
    cpu: { model: cpus[0]?.model?.trim() || 'Serverless CPU', cores: cpus.length || 1 },
    process: { pid: process.pid, uptime: fmt(process.uptime()), heapUsed: toMB(memUsage.heapUsed)+' MB', heapTotal: toMB(memUsage.heapTotal)+' MB', rss: toMB(memUsage.rss)+' MB', heapSizeLimit: toGB(heapStats.heap_size_limit)+' GB', env: process.env.NODE_ENV || 'production' },
    webUptime: fmt(Math.floor((Date.now() - _instanceStart) / 1000)),
    app: { activeSessions: sessionStore.size, framework: 'Express.js', runtime: process.env.NETLIFY ? 'Netlify Functions' : 'Node.js', serverless: !!process.env.NETLIFY },
